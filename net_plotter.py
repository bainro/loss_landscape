...
def set_weights(net, weights, directions=None, step=None):
    ...
    if directions is None:
        ...
        if len(directions) == 2:
            ...
            dz = directions[2]
            dt = directions[3]
            changes = [d0*step[0] + d1*step[1] + d2*step[2] + d3*step[3] for (d0, d1, d2, d3) in zip(dx, dy, dz, dt)]
        else:
...
def get_random_weights(weights):
    ...
    """
    _tmp = [torch.randn(w.size()) for w in weights]
    #print(np.sum(_tmp))
    #print(_tmp)
    return _tmp
...
def normalize_direction(direction, weights, norm='filter'):
    """
        ...
        Args:
          direction: a variable of the random direction for one layer
...
def create_random_direction(net, dir_type='weights', ignore='biasbn', norm='filter'):
    ...
    if dir_type == 'weights':
        ...
        print("INSIDE RANDOM DIR CREATION FX")
...
def setup_direction(args, dir_file, net):
    ...
    if not args.dir_file:
        ...
        if args.z:
            zdirection = create_random_direction(net, args.dir_type, args.zignore, args.znorm)
            h5_util.write_list(f, 'zdirection', zdirection)

        if args.t:
            if args.model_file2:
                tdirection = create_target_direction(net, net2, args.dir_type)
            else:
                tdirection = create_random_direction(net, args.dir_type, args.tignore, args.tnorm)
            h5_util.write_list(f, 'tdirection', tdirection)
...
def name_direction_file(args):
    ...
    if args.z:
        if args.zignore:
            dir_file += '_zignore=' + args.zignore
        if args.znorm:
            dir_file += '_znorm=' + args.znorm
    
    if args.t:
        if args.tignore:
            dir_file += '_tignore=' + args.tignore
        if args.tnorm:
            dir_file += '_tnorm=' + args.tnorm
