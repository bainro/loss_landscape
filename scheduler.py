...
def get_unplotted_indices(vals, xcoordinates, ycoordinates=None, zcoordinates=None, tcoordinates=None):
    """
    Args:
    ...
    if ycoordinates is not None:
        if zcoordinates is not None:
            if tcoordinates is not None:
                # If the plot is 3D + 1 time D
                xcoord_mesh, ycoord_mesh, zcoord_mesh, tcoord_mesh = np.meshgrid(xcoordinates, ycoordinates, zcoordinates, tcoordinates)
                s1 = xcoord_mesh.ravel()[inds]
                s2 = ycoord_mesh.ravel()[inds]
                s3 = zcoord_mesh.ravel()[inds]
                s4 = tcoord_mesh.ravel()[inds]
                return inds, np.c_[s1,s2,s3,s4]
            else:    
                # If the plot is 3D, then use meshgrid to enumerate all coordinates in the 3D mesh
                xcoord_mesh, ycoord_mesh, zcoord_mesh = np.meshgrid(xcoordinates, ycoordinates, zcoordinates)
                s1 = xcoord_mesh.ravel()[inds]
                s2 = ycoord_mesh.ravel()[inds]
                s3 = zcoord_mesh.ravel()[inds]
                return inds, np.c_[s1,s2,s3]
        else:    
...
def get_job_indices(vals, xcoordinates, ycoordinates, zcoordinates, tcoordinates, comm):
    """
    ...
    Args:
        ...
        xcoordinates: x locations, i.e.,[-1, -0.5, 0, 0.5, 1]
        ycoordinates: y locations, i.e.,[-1, -0.5, 0, 0.5, 1]
        zcoordinates: z locations, i.e.,[-1, -0.5, 0, 0.5, 1]
        tcoordinates: t locations, i.e.,[-1, -0.5, 0, 0.5, 1]
    ...
    inds, coords = get_unplotted_indices(vals, xcoordinates, ycoordinates, zcoordinates, tcoordinates)
